...
const CalendarWeekView = () => {
  ...
  return (
    <div className="grid relative overflow-auto h-full pr-2">
      <div className="grid grid-cols-8 sticky top-0 bg-card z-10 border-b mb-3">
        <div />
        {headerDays.map((date, i) => (
          <div
            key={date.toString()}
            className={cn(
              "font-medium text-center flex-1 gap-1 pb-2 text-sm flex items-center justify-center",
              [0, 6].includes(i) && "text-muted-foreground",
            )}
          >
            ...
            <span
              className={cn(
                ...
                isToday(date) &&
                  "bg-primary text-primary-foreground rounded-md size-6",
              )}
            >
              {format(date, "d")}
            </span>
      ...
      <div className="grid grid-cols-7 gap-px sticky top-0 bg-background border-b">
        {weekDays.map((day, i) => (
          <div
            key={day}
            className={cn(
              "mb-2 text-left text-sm font-medium pr-2",
              [0, 6].includes(i) && "text-muted-foreground",
            )}
          >
            {day}
          ...
          return (
            <div
              className={cn(
                ...
                isToday(_date) && "bg-accent",
              )}
              key={_date.toString()}
            >
              <span
                className={cn(
                  "size-6 grid place-items-center rounded mb-1 sticky top-0",
                  isToday(_date) &&
              ...
              {currentEvents.map((event) => {
                return (
                  ...
                  >
                    ...
                    <time className="tabular-nums text-muted-foreground text-xs">
...
const CalendarYearView = () => {
  ...
  const monthNames = useMemo(() => {
    return Array.from({ length: 12 }).map((_, i) => {
      return format(setMonth(new Date(), i), "MMMM", { locale });
    });
  }, [locale]);
  ...
  return (
    <TooltipProvider>
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-10 overflow-auto h-full pr-3">
        {months.map((days, i) => {
          return (
            <div key={days[0].toString()}>
              {/* Tampilkan nama bulan */}
              <span className="text-xl font-bold">{monthNames[i]}</span>
              ...
              <div className="grid gap-x-2 text-center grid-cols-7 tabular-nums">
                {days.map((_date) => {
                  const formattedDate = format(_date, "yyyy-MM-dd");
                  const isHoliday = holidays[formattedDate]?.holiday || false;
                  const holidaySummary = holidays[formattedDate]?.summary?.[0];

                  const isSunday = getDay(_date) === 0;

                  return (
                    <React.Fragment key={_date.toString()}>
                      {holidaySummary ? (
                        <Tooltip>
                              ...
                              <div
                                className={cn(
                                  ...
                                  isSameDay(today, _date) &&
                                    getMonth(_date) === i &&
                                    "bg-accent text-xl font-bold rounded-full",
                                )}
                              >
                                {format(_date, "d")}
                              </div>
                          ...
                          <TooltipContent>{holidaySummary}</TooltipContent>
                        </Tooltip>
                      ) : (
                        <div
                          key={_date.toString()}
  ...
  return (
    <div className="pr-2">
      {Array.from(Array(25).keys()).map((hour) => {
        return (
          <div className="text-right relative text-xs h-20 last:h-0" key={hour}>
            {now.getHours() === hour && (
              <div
                className="absolute z-10 left-full translate-x-2 w-[calc(100vw-655px)] h-[2px] dark:bg-red-400 bg-red-600"
                style={{
                  top: `${(now.getMinutes() / 60) * 100}%`,
                }}
              >
                <div className="size-2 rounded-full dark:bg-red-400 bg-red-600 absolute left-0 top-1/2 -translate-x-1/2 -translate-y-1/2"></div>
              </div>
            )}
