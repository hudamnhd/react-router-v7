...
export async function clientLoader({
  ...
  // }
  //
  const serverData = await ky.get("/resources/quran/xsurat").json();
  const data = serverData.find((d) => d.number_of_surah === parseInt(id));
...
export default function Route() {
  ...
  const surah_name = loaderData.name;
  const surah_number = loaderData.number_of_surah;
  const bookmarks_ayah = [];
  const lastRead = {};
  return (
    <div className="relative  max-w-4xl mx-auto w-full border-x">
      <div className="p-1.5 flex justify-between gap-x-3 border-b">
        ...
        >
          ...
          <span className="text-lg font-semibold">
            {surah_number}. {surah_name} ({loaderData.name_translations.id})
          </span>
        </Link>
        {/*<Link
          className={cn(
      ...
      <div className="">
        {loaderData?.verses?.map((_dt) => {
          const dt = {
            ..._dt,
            surah_name,
          };
          const isFavorite = bookmarks_ayah.some((fav) => fav.id === dt.number);
          const isLastRead = lastRead?.id === dt.number;

          return (
            <div className="border-t" key={dt.number}>
              <div
                className={cn(
              ...
              >
                ...
                {opts?.font_translation === "on" && (
                  <div className="translation-text">
                    <div className="max-w-none prose-normal duration-300 text-muted-foreground mb-2">
                      {dt.translation_id}
                    </div>
                  </div>
                )}
...
//   renderCount.current++;
//   const { group_surat, id, quranData } = useLoaderData();
//   console.warn("DEBUGPRINT[6]: $id.tsx:125: quranData=", quranData);
//   const loaderRoot = useRouteLoaderData("root");
//   const opts = loaderRoot?.opts || {};
